...
new_codes = ['241520','440290','309960','019550','289080','246690','293580','027830',
             '277070','100790','330730','026890','027360','021080','041190','452300',
             '307930','016600','019570']

prev_ref_code = '005830'  # DB손해보험 (앞서 분석한 보험사)
...
# 1) 신규 19개 종목 내부 비교
out += compare_group(new_codes, '신규 19개 종목 내부 비교', results)

# 2) 신규 1개 vs 이전 종목 비교
sample_new = new_codes[0]
out += compare_group(
    [sample_new, prev_ref_code],
    '신규(' + sample_new + ' ' + results[sample_new]['종목명'] + ') vs 이전(' + prev_ref_code + ' ' + results[prev_ref_code]['종목명'] + ')',
    results
)

output = '\n'.join(out)
with open('temp_comp/structure_check5.txt', 'w', encoding='utf-8') as f:
    f.write(output)
print('\nsaved to temp_comp/structure_check5.txt')
