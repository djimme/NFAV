...
for tab_name in ['손익Y', '손익Q', '대차Y', '대차Q', '현금Y', '현금Q']:
    ...
    if diff_r:
        out.append(f'  기준행({ref_code} {results[ref_code]["종목명"]}): {len(ref_r)}행')
        ...
        for c, r in diff_r.items():
            ...
            if only_in_ref:
                out.append(f'     기준에만 있는 항목: {sorted(only_in_ref)}')
            if only_in_cur:
                out.append(f'     {c}에만 있는 항목: {sorted(only_in_cur)}')
    else:
        out.append(f'  행({len(ref_r)}개): 전체 동일')
        out.append(f'  항목: {ref_r}')

output = '\n'.join(out)
with open('temp_comp/structure_check2.txt', 'w', encoding='utf-8') as f:
    f.write(output)
print('saved to temp_comp/structure_check2.txt')
