...
업종별 재무비율 수집:
- 연결 누적 (Annual)  : 연도별 재무비율 → 컬럼 형식 {YYYY(누적)}_{지표명}
...
Public API:
  ...
  collectFinanceRatio(code)  : HTML 가져오기 + 파싱 통합 수집 → dict 또는 None
"""
import re
...
def parseFnguideFiRatio(html):
    ...
    # 연결 누적 (Annual) — 컬럼 형식: {YYYY(누적)}_{지표명}, 연도별 최신 기간, 최근 4개년
...
def collectFinanceRatio(code):
    """
    FinanceRatio HTML 가져오기 + 파싱 통합 수집
    ...
    Returns:
        ...
        None: 연결 누적 테이블이 없는 경우
    """
    return parseFnguideFiRatio(getFnGuideFiRatio(code))
...
def _parse_grid_section(soup, grid_id, period_format='year', max_periods=None):
    ...
    Args:
        ...
        period_format: 'year' → {YYYY(누적)} (연도별 최신 기간 사용), 'yearmonth' → {YYYY/MM}
